...
export function InventoryPreview({
  ...
  /** Fallback icons only for slots missing from slots_preview (legacy builds). */
  const [fallbackIcons, setFallbackIcons] = useState<Record<string, string | null>>({});
  ...
  useEffect(() => {
    ...
    const missing = Object.entries(slots).filter(
      (entry): entry is [string, number] =>
        entry[1] != null && !slotsPreview?.[entry[0]],
    );
    if (missing.length === 0) {
      setFallbackIcons({});
      return;
    ...
    void (async () => {
      ...
      await Promise.all(
        missing.map(async ([slotKey, itemId]) => {
      ...
      if (!cancelled) setFallbackIcons(next);
    })();

    return () => { cancelled = true; };
  }, [slots, slotsPreview]);

  /** Prefer slots_preview (instant) — fetch only as hover / legacy fallback. */
  function resolveIconUrl(slotKey: string): string | null {
    return slotsPreview?.[slotKey] ?? fallbackIcons[slotKey] ?? null;
  }
    ...
    if (showLabels) {
      return (
        <div
          ...
          onMouseEnter={canHover ? (e) => handleHoverEnter(slotKey, e) : undefined}
          onMouseMove={canHover ? move : undefined}
          onMouseLeave={canHover ? handleHoverLeave : undefined}
        >
  ...
  return (
    <>
            ...
            {BOOK_DOFUS_SLOTS.map((s) => {
              ...
              return (
                <div key={s}
                  ...
                  onMouseEnter={canHover ? (e) => handleHoverEnter(s, e) : undefined}
                  onMouseMove={canHover ? move : undefined}
                  onMouseLeave={canHover ? handleHoverLeave : undefined}
                >
                  {url ? (
      ...
      {openSetId != null && (
        <SetDetailModal setId={openSetId} onClose={closeSet} />
      )}
    </>
  );
}
